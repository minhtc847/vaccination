...
const SelectSchedule: React.FC = () => {
    ...
    document.title = "Select Injection Schedule"
    ...
    return (
        <Container className='pt-4'>
            ...
            <h5 className='pb-3' style={{ fontWeight: "bold" }}>SELECT INJECTION SCHEDULE</h5>
                        ...
                        <table>
                            <thead>
                                <tr>
                                    <th>Vaccine</th>
                                    <th>Time</th>
                                    <th style={{ width: '100px' }}>Inject per day</th>
                                    <th>Place</th>
                                    <th>Status</th>
                                    <th>Note</th>
                                    <th>Actions</th>
                            ...
                            <tbody className='text-start'>
                                {schedule.map(s => (
                                    <tr key={s.id}>
                                        ...
                                        <td style={{ width: '100px' }}>{s.injectionTimes}</td>
                                        <td>{s.place}</td>
                                        <td>{s.status}</td>
                                        <td>{s.description?.length > 30 ? `${s.description.substring(0, 30)}...` : s.description}</td>
                                        <td>
                                            {s.status === 'Open' ?
                                                <Link to={`/result/create/${s.id}`} className='custom-id'>Choose</Link>
                                                : null}
                                        </td>
                                    </tr>
                                ))}
